...
date: '2024-12-09'
description: "Have you ever wondered how your laptop server and Google's server communicate with each other?"
...
Data: “How will I travel from your laptop to YouTube’s server?”

Transport Layer: “Hold my beer.” 🍺
...
The Link Layer controls how devices on the same network segment communicate - like your PC and home router when you connect to watch Stranger Things Season 5 😄.
...
The 3-way handshake:
- _“Hi, can we talk?”_ (SYN)
- _“Yes, I can talk. Can you?”_ (SYN-ACK)
- _“Yes, let’s go baby.”_ (ACK)
...
If the sender transmits too fast, the receiver’s buffer may overflow this leads to packet loss.
...
And somehow… it works great for tons of real-time applications.

No handshake, no checking before sending the message, no ordering, no reliability, just sending and finishing its work.

`UDP header → generally small (8 bytes)`

`TCP header → larger in size due to extra information (20 bytes)`
Which means:

### That’s why UDP is used in:
- Video calls
- Gaming
- Live streaming
- DNS queries
...
It is not that the “lazy one” is not useful, it is incredibly useful.

#### Real-time communication
- Online Gaming (PUBG, Valorant)
	- One update lost → okay
	- TCP retransmission → lag & stutter
- Video calls
...
- DNS Query
	- UDP is enough
	- If one query fails, just send again (Ctrl C + Ctrl V 😄)

---

### When to use TCP?

- File Download and Upload → must be 100% correct
- Email → Cannot be delivered partially
- Database queries & APIs → Must be accurate
- Financial transactions → No room for mistakes
